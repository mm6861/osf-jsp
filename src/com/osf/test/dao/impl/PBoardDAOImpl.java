...
public class PBoardDAOImpl implements PBoardDAO{
	private static final String INSERT_PBOARD = "insert into photo_board(pb_num,pb_title,pb_content,pb_credat,pb_cretim,pb_real_path,pb_file_path)"
			+ " values(seq_pb_num.nextval,?,?,to_char(sysdate,'YYYYMMDD'),TO_CHAR(sysdate,'HH24MISS'),?,?)";
	private static final String SELECT_PBOARD = "select * from photo_board";
	...
	public int insertPBoard(Map<String, String> pBoard) {
		...
		try {
			ps = DBCon.getCon().prepareStatement(INSERT_PBOARD);
	...
	public Map<String, String> selectPBoard(int pbNum) {
		try {
			PreparedStatement ps = DBCon.getCon().prepareStatement(SELECT_PBOARD_BY_PB_NUM);
			ps.setInt(1, pbNum);
			ResultSet rs = ps.executeQuery();
			while(rs.next()) {
				Map<String,String> pBoard = new HashMap<>();
